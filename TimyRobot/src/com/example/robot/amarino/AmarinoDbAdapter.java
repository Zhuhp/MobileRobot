...
public class AmarinoDbAdapter {
    ...
    public boolean deleteDevice(long deviceId) {
    	...
    	if (DEBUG) Log.d(TAG, "delete device with id " + deviceId + ": " + numEvents + " associated events removed");
    ...
    public Event getEvent(long deviceId, int pluginId){
    	...
    	if (c == null){
        	if (DEBUG) Log.d(TAG, "no event found for device with id: " + deviceId + " and pluginId:" + pluginId);
        	return null;
        }
        if (c.moveToFirst()){
    ...
    public ArrayList<Event> fetchEvents(long deviceId) {
        ...
        if (c == null){
        	if (DEBUG) Log.d(TAG, "no events found for device with id: " + deviceId);
        	return events;
        }
        if (c.moveToFirst()){	
        	do {
        		...
        		events.add(e);
        		if (DEBUG) Log.d(TAG, "event found: " + e.name + " - id=" + e.pluginId);
        	}
        	while(c.moveToNext());
        }
        else {
