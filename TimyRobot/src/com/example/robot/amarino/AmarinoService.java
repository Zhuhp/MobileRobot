...
*/
package com.example.robot.amarino;
...
import com.example.robot.view.MainActivity;
...
//		super.onStart(intent, startId);
		...
		if (address == null) {
			Log.d(TAG, "EXTRA_DEVICE_ADDRESS not found!");
			return START_NOT_STICKY;
		}
	...
	public void forwardDataToArduino(Intent intent){
		...
		if (pluginId == -1) {
			...
			if (address == null) {
				Log.d(TAG, "Data not sent! EXTRA_DEVICE_ADDRESS not set.");
				return;
			}
			...
			// cutoff leading flag and ACK_FLAG for Log
			Log.d(TAG, getString(R.string.service_message_to_send, message.substring(1, message.length()-1)));
			...
			if (devices != null && devices.size() != 0){
				...
				Log.d(TAG, "No device associated with plugin: " + pluginId);
			}
		}
	}
	...
	private void shutdownService(boolean disablePlugins){
		...
		if (serviceState == NO_CONNECTIONS){
			...
			Log.d(TAG, getString(R.string.service_ready_to_shutdown));
			stopSelf();
		}
	}
			...
			for (Event e : events){
				if (enable) {
					...
					if (devices != null) {
						...
						Log.d(TAG, "disable requested for Plugin " + e.name + " detected, but was never enabled");
	...
	private void informPlugIn(Event e, String address, boolean enable){
		Log.d(TAG, (enable ? getString(R.string.enable) : getString(R.string.disable)) + " " + e.name);
		Intent intent;
		if (enable)
	...
	private void sendPairingRequested(String address){
		Log.d(TAG, getString(R.string.service_pairing_request, address));
	...
	public class AmarinoServiceBinder extends Binder {
		public AmarinoService getService() {
	...
	private void initNotificationManager() {
		...
		launchIntent = PendingIntent.getActivity(AmarinoService.this, 0, 
				new Intent(AmarinoService.this, MainActivity.class)
	      	...
	      	try {
	    		...
	    		if (!isPaired){
		    		...
		    		try {
						...
						Log.d(TAG, "Connection via SDP unsuccessful, try to connect via port directly");
	    ...
	    public void run() {
	        ...
	        while (true) {
	            try {
	            	...
	            	Log.d(TAG, "communication to " + mAddress + " halted");
	                break;
	            }
	        }
	    }
...
//					}
	    ...
	    public void write(byte[] bytes) {
	        try {
	            ...
	            Log.d(TAG, "send to Arduino: " + new String(bytes));
	        } catch (IOException e) { }
	    }
